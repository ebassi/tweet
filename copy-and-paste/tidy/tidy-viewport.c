...
static void
tidy_viewport_request_coords (ClutterActor *self, ClutterActorBox *box)
{
  ClutterFixed prev_value;
  
  TidyViewportPrivate *priv = TIDY_VIEWPORT (self)->priv;

  /* Chain up to get group size */
  CLUTTER_ACTOR_CLASS (tidy_viewport_parent_class)->request_coords (self, box);
...
tidy_viewport_class_init (TidyViewportClass *klass)
{
  ...
  actor_class->request_coords = tidy_viewport_request_coords;
